...
public class RenderingWidget : SpriteRenderingWidget
{
	...
	public RenderingWidget ( MainWindow window, Widget parent ) : base( parent )
	{
		...
		OriginMarker = new Draggable( GizmoInstance.World, "models/preview_quad.vmdl", Transform.Zero );
	...
	protected override void OnMousePress ( MouseEvent e )
	{
		...
		if ( e.LeftMouseButton )
		{
			...
			var tr = GizmoInstance.World.Trace.Ray( Camera.ScreenPixelToRay( cursorLocalPos ), 8000f ).WithTag( "draggable" ).Run();
	...
	protected override void OnMouseMove ( MouseEvent e )
	{
		...
		{
			var cursorLocalPos = Editor.Application.CursorPosition - ScreenRect.Position;
			var tr = GizmoInstance.World.Trace.Ray( Camera.ScreenPixelToRay( cursorLocalPos ), 5000f ).WithTag( "draggable" ).Run();
	...
	protected override void PreFrame ()
	{
		...
		float scale = Camera.OrthographicHeight / 1024f;
		...
		foreach ( var attachment in MainWindow.SelectedAnimation?.Attachments ?? new List<SpriteAttachment>() )
		{
			...
			if ( !has )
			{
				...
				attach = new Draggable( GizmoInstance.World, "models/preview_quad.vmdl", Transform.Zero );
